...
SKCM_RNAseq <- read_delim("~/Yardena/SKCM.rnaseqv2__illuminahiseq_rnaseqv2__unc_edu__Level_3__RSEM_genes_normalized__data.data.txt",
"\t", escape_double = FALSE, trim_ws = TRUE)
SKCM_RNAseq = as.data.frame(SKCM_RNAseq)
SKCM_RNAseq$`Hybridization REF` = sapply(SKCM_RNAseq$`Hybridization REF`, function(x) strsplit(x,split = "\\|")[[1]][1])
SKCM_RNAseq = SKCM_RNAseq[-1,]
colnames(SKCM_RNAseq)[2:474] = substr(colnames(SKCM_RNAseq)[2:474],1,12)
SKCM_RNAseq[,2:474] = apply(SKCM_RNAseq[,2:474],2,as.numeric)
CYT = apply(SKCM_RNAseq[,2:474], 2, function(x){
    idx1 = which(SKCM_RNAseq$`Hybridization REF` == "GZMA")
    idx2 = which(SKCM_RNAseq$`Hybridization REF` == "PRF1")
    return(exp((log(x[idx1]+0.01) + log(x[idx2]+0.01))/2))
})
...
#TCGA survival analysis Figure 6
...
survdat = data.frame(time = clinical_TCGA[common,5],                                          
                  ...
                  ITH = TCGA_data[common,8],
                  diversity = TCGA_data[common,11],
                  MUTATION_LOAD = TCGA_data[common,9],
                  CNV_LOAD = TCGA_data[common,10],
                  SIG7 = TCGA_data[common,12],
                  CYT = log(CYT[common]))
